...
from .cifar_runners import run_cifar_em_latent_matched_sft


def main() -> None:
    parser = argparse.ArgumentParser(description='EM-style latent-matched SFT on CIFAR-10.')
    parser.add_argument('--generator_ckpt', type=str, required=True)
    parser.add_argument('--data_root', type=str, default='./data')
    parser.add_argument('--output_dir', type=str, default='outputs/cifar_em_latent_matched_sft')
    parser.add_argument('--em_rounds', type=int, default=3)
    parser.add_argument('--assign_max_items', type=int, default=512)
    ...
    run_cifar_em_latent_matched_sft(**vars(args))
