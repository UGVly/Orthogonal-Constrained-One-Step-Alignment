...
from .mnist_data import AssignedMNISTDataset, MNISTTargetDataset, assigned_mnist_collate, save_tensor_image
from .mnist_models import MNISTConditionalDiscriminator, MNISTConditionalGenerator, sample_mnist_grid
...
def _load_generator_checkpoint(generator: MNISTConditionalGenerator, checkpoint_path: str, device: str) -> None:
...
@torch.no_grad()
def _save_eval_grid(generator: MNISTConditionalGenerator, output_path: str, device: str, z: Optional[torch.Tensor] = None) -> None:
    grid = sample_mnist_grid(generator, device=device, z=z)
    save_grid(grid, output_path)


def train_mnist_one_step_gan(
    *,
    output_dir: str,
    data_root: str = './data',
    batch_size: int = 128,
    ...
    dataset = MNISTTargetDataset(root=data_root, train=True, limit=train_limit)
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, drop_last=True)

    generator = MNISTConditionalGenerator(z_dim=z_dim).to(dev)
    discriminator = MNISTConditionalDiscriminator().to(dev)
    ...
    for epoch in range(1, epochs + 1):
        pbar = tqdm(loader, desc=f'MNIST GAN epoch {epoch}/{epochs}')
        for batch in pbar:
            global_step += 1
        ...
        torch.save(ckpt, os.path.join(output_dir, f'mnist_gan_epoch_{epoch:03d}.pt'))

    torch.save({'generator': generator.state_dict(), 'discriminator': discriminator.state_dict(), 'z_dim': z_dim}, os.path.join(output_dir, 'mnist_gan_final.pt'))
    _save_history_plot(history, os.path.join(output_dir, 'mnist_gan_loss_history.png'))
    _save_eval_grid(generator, os.path.join(output_dir, 'mnist_gan_final_samples.png'), dev, z=fixed_z)


def build_mnist_assigned_noise_dataset(
    *,
    generator_ckpt: str,
    output_dir: str,
    data_root: str = './data',
    ...
    set_seed(seed)
    dev = get_device(device)
    dataset = MNISTTargetDataset(root=data_root, train=train_split, limit=max_items)
    ...
    with open(manifest_path, 'w', encoding='utf-8') as manifest_f:
        for item_idx in tqdm(range(len(dataset)), desc='Assign matched MNIST noise'):
            ...
            target_path = os.path.join(images_dir, f'{item_idx:06d}_label{item["label"]}.png')
            recon_path = os.path.join(recons_dir, f'{item_idx:06d}_label{item["label"]}.png')
            noise_path = os.path.join(noises_dir, f'{item_idx:06d}_label{item["label"]}.pt')
            base_noise_path = os.path.join(base_noises_dir, f'{item_idx:06d}_label{item["label"]}.pt')
            orth_path = os.path.join(orth_dir, f'{item_idx:06d}_label{item["label"]}.pt')
            curve_path = os.path.join(noises_dir, f'{item_idx:06d}_curve.json')
            save_tensor_image(item['image'], target_path)
            if best_recon is not None:
                save_tensor_image(best_recon[0], recon_path)
            torch.save(best_z, noise_path)
            torch.save(best_base_noise, base_noise_path)
            torch.save(best_q, orth_path)
            with open(curve_path, 'w', encoding='utf-8') as f:
                json.dump({'loss_curve': curve, 'best_loss': best_loss, **result.metrics}, f, indent=2, ensure_ascii=False)
    ...
    _save_history_plot(history, os.path.join(output_dir, 'assigned_noise_loss_summary.png'))
    ...
    with open(os.path.join(output_dir, 'assigned_noise_summary.json'), 'w', encoding='utf-8') as f:
        json.dump({
            'num_items': len(dataset),
            'mean_best_loss': float(sum(history['best_loss']) / max(len(history['best_loss']), 1)),
            'max_best_loss': float(max(history['best_loss'])) if history['best_loss'] else 0.0,
...
@torch.no_grad()
def _make_teacher(generator_ckpt: str, z_dim: int, device: str) -> MNISTConditionalGenerator:
    teacher = MNISTConditionalGenerator(z_dim=z_dim).to(device)
...
def _save_recon_grid(
    ...
    for i in range(count):
        item = dataset[i]
        zs.append(item['noise'])
        ys.append(item['label'])
    z = torch.stack(zs, dim=0).to(device)
    y = torch.tensor(ys, dtype=torch.long, device=device)
    recon = generator(z, y)
    save_grid((recon + 1.0) * 0.5, output_path)


def train_mnist_latent_matched_sft(
    *,
    generator_ckpt: str,
    manifest_path: str,
    output_dir: str,
    ...
    dataset = AssignedMNISTDataset(manifest_path)
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, collate_fn=assigned_mnist_collate)

    student = MNISTConditionalGenerator(z_dim=z_dim).to(dev)
    ...
    for epoch in range(1, epochs + 1):
        pbar = tqdm(loader, desc=f'MNIST latent-matched SFT epoch {epoch}/{epochs}')
        for batch in pbar:
        ...
        torch.save({'generator': student.state_dict(), 'epoch': epoch, 'z_dim': z_dim}, os.path.join(output_dir, f'mnist_latent_matched_sft_epoch_{epoch:03d}.pt'))

    torch.save({'generator': student.state_dict(), 'z_dim': z_dim}, os.path.join(output_dir, 'mnist_latent_matched_sft_final.pt'))
    _save_history_plot(history, os.path.join(output_dir, 'mnist_latent_matched_sft_loss_history.png'))
    _save_eval_grid(student, os.path.join(output_dir, 'mnist_latent_matched_sft_final_random_samples.png'), dev, z=fixed_z)
    _save_recon_grid(student, dataset, os.path.join(output_dir, 'mnist_latent_matched_sft_final_assigned_recons.png'), dev)


def train_mnist_direct_random_noise_sft(
    *,
    generator_ckpt: str,
    data_root: str,
    output_dir: str,
    ...
    for epoch in range(1, epochs + 1):
        pbar = tqdm(loader, desc=f'MNIST direct random-noise SFT epoch {epoch}/{epochs}')
        for batch in pbar:
            target = batch['image'].to(dev)
            labels = batch['label'].to(dev) if isinstance(batch['label'], torch.Tensor) else torch.tensor(batch['label'], dtype=torch.long, device=dev)
            bs = target.shape[0]
        ...
        _save_eval_grid(student, os.path.join(output_dir, f'epoch_{epoch:03d}_random_samples.png'), dev, z=fixed_z)
        torch.save({'generator': student.state_dict(), 'epoch': epoch, 'z_dim': z_dim}, os.path.join(output_dir, f'mnist_direct_sft_epoch_{epoch:03d}.pt'))

    torch.save({'generator': student.state_dict(), 'z_dim': z_dim}, os.path.join(output_dir, 'mnist_direct_sft_final.pt'))
    _save_history_plot(history, os.path.join(output_dir, 'mnist_direct_sft_loss_history.png'))
    _save_eval_grid(student, os.path.join(output_dir, 'mnist_direct_sft_final_random_samples.png'), dev, z=fixed_z)


def run_mnist_em_latent_matched_sft(
    *,
    generator_ckpt: str,
    data_root: str,
    output_dir: str,
    em_rounds: int = 3,
    assign_max_items: int = 256,
    assign_steps: int = 200,
    assign_lr: float = 5e-2,
    sft_epochs: int = 2,
    ...
    for round_idx in range(1, em_rounds + 1):
        ...
        build_mnist_assigned_noise_dataset(
        ...
        train_mnist_latent_matched_sft(
            generator_ckpt=current_ckpt,
        ...
        current_ckpt = os.path.join(sft_dir, 'mnist_latent_matched_sft_final.pt')
